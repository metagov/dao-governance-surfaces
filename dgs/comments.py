...
def clean_comment_block(lines: list[str]) -> list[str]:
    ...
    match = re.search(PATTERN_SEARCH_COMMENTBLOCK, linesStr)
    if match:
        # Remove any asterisks
        lines_new = match.group(1).split('\n')
        lines_new = [re.sub(PATTERN_SUB_ASTERISKS, '', s).strip() for s in lines_new if s]
    else:
        ...
        while i >= 0 and not endFlag:
            if lines[i].startswith(LINE_COMMENT_MARKER):
                lines_new.append(re.sub(rf'{{{LINE_COMMENT_MARKER}}}+', '', lines[i]).strip())
            else:
                endFlag = True
            i -= 1
...
def clean_inline_comment(line: str) -> str:
    ...
    split_str = re.split(PATTERN_LINE_COMMENT, line)
    inline_comment = [split_str[-1]] if len(split_str) == 2 else ''

    return inline_comment


def clean_comment_lines(lines_raw: list[str], includes_inline: bool = False) -> list[str]:
    ...
    if includes_inline:
        ...
        block_comment = clean_comment_block(lines_raw[:-1])
        inline_comment = clean_inline_comment(lines_raw[-1])
        lines_new = block_comment + [inline_comment]
    else:
        # Treat lines as block comment
        lines_new = clean_comment_block(lines_raw)
...
def parse_object_comments(lines_raw: list[str]) -> dict:
    ...
    lines = clean_comment_lines(lines_raw, includes_inline=False)  
...
def parse_parameter_comments(lines_raw: list[str]) -> dict:
    ...
    # Clean lines and skip the rest if no comment found
    lines = clean_comment_lines(lines_raw, includes_inline=True)
...
def add_docstring_comments(lines: list[str], df_objects: DataFrame, df_parameters: DataFrame) -> DataFrame:
    ...
    for i, row in df_o.iterrows():
        ...
        comment_data = parse_object_comments(commentLines)
...
def add_inline_comments(lines: list[str], df_parameters: DataFrame) -> DataFrame:
    ...
    for i, row in df_p.iterrows():   
        ...
        comment_data = parse_parameter_comments(commentLines)
...
def remove_duplicate_comments_in_parameters(df_o: DataFrame, df_parameters: DataFrame) -> DataFrame:
